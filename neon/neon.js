...
elements.bgSelect.addEventListener('change', function() {
    ...
    document.body.style.background = ''; // Clear inline styles
    
    if (this.value === 'custom') {
      elements.bgColor.style.display = 'inline-block';
      document.body.classList.add('bg-custom');
      document.body.style.setProperty('--custom-bg', elements.bgColor.value);
    } else {
      elements.bgColor.style.display = 'none';
      document.body.classList.add(`bg-${this.value}`);
    }
  });
  
  // Custom color picker handler
  elements.bgColor.addEventListener('input', function() {
    ...
    function applyNeonEffect(settings) {
        ...
        // Apply animation
        neonPreview.className = 'neon-text';
        if (settings.animation !== 'none') {
            neonPreview.classList.add(settings.animation);
            neonPreview.style.setProperty('--animation-speed', `${settings.animationSpeed}s`);
        }
...
// Background buttons functionality
document.querySelectorAll('.bg-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        if (this.dataset.bg === 'custom') return;
        ...
        document.body.classList.add(`bg-${this.dataset.bg}`);
        
        // Hide color picker if not custom
...
document.getElementById('bg-color').addEventListener('input', function() {
    if (document.querySelector('.bg-btn[data-bg="custom"]').classList.contains('active')) {
        document.body.style.setProperty('--custom-bg', this.value);
    }
});

// Initialize with dark background
document.querySelector('.bg-btn[data-bg="dark"]').click();